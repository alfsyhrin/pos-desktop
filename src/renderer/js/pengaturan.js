...
/* ======================================================
 * HELPER: SHOW NOTIFICATION (GLOBAL, DIPAKAI DI SEMUA BAGIAN)
 * ====================================================== */
...
function renderStoreAccordionsForOwner(stores) {
  ...
  if (!stores || stores.length === 0) {
    container.innerHTML =
      '<p style="color: var(--paragraph-color); padding: 16px;">Tidak ada toko yang ditemukan</p>';
    return;
  }
...
/* ======================================================
 * RENDER SINGLE STORE UNTUK ADMIN & CASHIER
 * ====================================================== */
...
function setupModalEvents(modal, cancelBtn, saveBtn) {
  ...
  function openModal(storeId) {
    ...
    const inputReceiptTemplate = document.getElementById('editReceiptTemplate');
    ...
    if (inputReceiptTemplate) inputReceiptTemplate.value = store.receipt_template || '';
...
async function updateStoreInfo(storeId) {
  ...
  const receipt_template =
    document.getElementById('editReceiptTemplate')?.value.trim() || '';
  ...
  try {
    const res = await fetch(`http://103.126.116.119:8001/api/stores/${storeId}`, {
      ...
      body: JSON.stringify({ name, address, phone, tax_percentage, receipt_template })
    });
...
function setupOwnerModalEvents(editBtn, modal, cancelBtn, saveBtn) {
  ...
  editBtn.addEventListener('click', function (e) {
    e.preventDefault();
    e.stopPropagation();
    document.getElementById('editOwnerBusinessName').value =
      document.getElementById('ownerBusinessName').textContent.trim();
    document.getElementById('editOwnerEmail').value =
      document.getElementById('ownerEmail').textContent.trim();
    document.getElementById('editOwnerPhone').value =
      document.getElementById('ownerPhone').textContent.trim();
    document.getElementById('editOwnerAddress').value =
      document.getElementById('ownerAddress').textContent.trim();
...
async function updateOwnerInfo() {
  ...
  const business_name =
    document.getElementById('editOwnerBusinessName')?.value.trim() || '';
...
/* ======================================================
 * BACKUP / IMPORT / RESET DATA + MODAL EXPORT
 * ====================================================== */
document.addEventListener('DOMContentLoaded', () => {
  console.log('🔧 init backup/import/reset');

  const modalExport = document.getElementById('modalExportData');
  const cancelExportBtn = document.getElementById('cancelExportBtn');
  const confirmExportBtn = document.getElementById('confirmExportBtn');

  const cardBackupExport = document.querySelector('.card-backup-export');
  const cardBackupImport = document.querySelector('.card-backup-import');
  const cardResetData = document.querySelector('.card-reset-data');

  console.log('modalExport =', modalExport);
  console.log('cardBackupExport =', cardBackupExport);
  console.log('cardBackupImport =', cardBackupImport);
  console.log('cardResetData =', cardResetData);

  // --- EXPORT (Backup ke file) ---
  if (cardBackupExport && modalExport) {
    cardBackupExport.addEventListener('click', () => {
      console.log('Backup card clicked');
      modalExport.classList.add('active');
      console.log('modalExport classList =', modalExport.className);
    });
  }

  if (cancelExportBtn && modalExport) {
  ...
  if (confirmExportBtn && modalExport) {
    confirmExportBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const url = `http://103.126.116.119:8001/api/backup/export?data=${dataType}&type=${fileType}`;
  ...
  // --- IMPORT (Restore dari file) ---
  if (cardBackupImport) {
    cardBackupImport.addEventListener('click', () => {
      ...
      input.onchange = async () => {
        ...
        if (file.name.endsWith('.zip')) {
          showNotification(
            'Import ZIP belum didukung. Silakan upload file .json, .csv, atau .xlsx',
            'warning'
          );
          input.remove();
          return;
  ...
  if (cardResetData) {
    cardResetData.addEventListener('click', async () => {
...
console.log('📌 end of pengaturan.js reached');
